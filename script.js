...
<script>
    window.onload = function() {
        ...
        const loginContainer = document.getElementById('login-container');
        const gameAndProfileContainer = document.getElementById('game-and-profile-container');
        const withdrawalPageContainer = document.getElementById('withdrawal-page-container');
        const userNameInput = document.getElementById('user-name-input');
        const startButton = document.getElementById('start-button');
        const logoutButton = document.getElementById('logout-button');
        const loginMessageBox = document.getElementById('login-message-box');
        ...
        // New elements for the withdrawal page
        const rewardsClaimedContainer = document.getElementById('rewards-claimed-container');
        const backToHomeButton = document.getElementById('back-to-home-button');
        const currentPointsWithdrawal = document.getElementById('current-points-withdrawal');
        ...
        const dailyTasks = [
            { id: 'tap-task', text: 'Tap 50 times', points: 10, completed: false, check: () => tapCount >= 50 },
            { id: 'reward-task', text: 'Daily bonus 5Rs', points: 5, completed: false, check: () => rewardsClaimed >= 1 },
            { id: 'video-task', text: 'Subscribe the channel 8Rs', points: 8, completed: false, check: () => videoWatched },
            { id: 'youtube-task', text: 'Subscribe to our YouTube channel 8Rs', points: 8, completed: false, check: () => youtubeSubscribed }
        ];

        const withdrawalOptions = [
            { points: 100, amount: 10 },
            { points: 200, amount: 20 },
        ...
        function renderWithdrawalOptions() {
            ...
            currentPointsWithdrawal.textContent = points;
            ...
            withdrawalOptions.forEach(option => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button data-points="${option.points}" class="withdraw-button px-4 py-2 text-sm font-semibold rounded-lg shadow-md transition-all duration-200 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed">
                            Withdraw
                        </button>
                    </td>
                `;
            ...
            document.querySelectorAll('.withdraw-button').forEach(button => {
                const withdrawalPoints = parseInt(button.dataset.points);
                if (points >= withdrawalPoints) {
                ...
                button.addEventListener('click', (event) => {
                    handleWithdrawal(withdrawalPoints);
                });
            });
        }

        function handleWithdrawal(withdrawalPoints) {
            if (points >= withdrawalPoints) {
                points -= withdrawalPoints;
                rewardsClaimed++; // Consider a separate counter for withdrawals if needed
                updateDisplays();
                renderWithdrawalOptions();
                saveGameState();
                // You would add logic here to trigger a real withdrawal process (e.g., show a success message, an input field for UPI ID, etc.)
                alert(`Successfully withdrew for ${withdrawalPoints} points! Your new balance is ${points}.`);
            }
        }
        ...
        function showLoginView() {
            loginContainer.classList.remove('hidden');
            gameAndProfileContainer.classList.add('hidden');
            withdrawalPageContainer.classList.add('hidden');
        }
        ...
        function showWithdrawalView() {
            loginContainer.classList.add('hidden');
            gameAndProfileContainer.classList.add('hidden');
            withdrawalPageContainer.classList.remove('hidden');
            renderWithdrawalOptions();
        }
        ...
        function resetGameState() {
            ...
            userNameInput.value = '';
            dailyTasks.forEach(task => task.completed = false);
            localStorage.removeItem('tapAndEarnGameState');
            updateDisplays();
            checkRewardStatus();
            renderDailyTasks();
            profileOptions.classList.add('hidden');
        ...
        function updateDisplays() {
            pointsDisplay.textContent = points;
            // The combined display for rewards
        ...
        // --- New Event Listeners for Withdrawal Page ---
        rewardsClaimedContainer.addEventListener('click', () => {
            showWithdrawalView();
        });
